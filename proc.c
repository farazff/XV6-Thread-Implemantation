...
int clone(int (*fn)(void *, void*), void *stack)
{
  ...
  newProc->tgid = curproc->tgid;
  newProc->process = curproc->process;
  newProc->threadcount = 0;
   
  initlock(&newProc->vlock, "vlock");
  ...
  *newProc->tf = *curproc->tf;
  newProc->parent = curproc->parent;
   
  sp = (uint *)stack;
  sp -= 1;
  *sp = (uint)0xffffffff;
   
  newProc->tf->eip = (uint)fn;
  newProc->tf->esp = (uint)sp;
	newProc->cwd = 0;
  ...
  safestrcpy(newProc->name, curproc->name, strlen(curproc->name));
